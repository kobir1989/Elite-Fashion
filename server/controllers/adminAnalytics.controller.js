 ...
 ******************************************************************************/
module.exports.getAdminAnalyticsData = catchAsync(async (req, res) => {
  ...
  const totalUsersPercentage =
    prevTotalUsers !== 0 ? (totalUsers / prevTotalUsers - 1) * 100 : 0
  const totalOrdersPercentage =
    prevTotalOrders !== 0 ? (totalOrders / prevTotalOrders - 1) * 100 : 0

  const [dailySales, weeklyEarnings, monthlyRevenue, totalRevenue] =
    await Promise.all([
  ...
  orders.forEach(order => {
    ...
    const index = monthlyRevenueArray.findIndex(
      monthData => monthData.month === orderMonth
    )
    if (index >= 0) {
  ...
  const weeklyEarningsPercentage = Math.round(
    (weeklyEarnings / totalRevenue) * 100
  )
  const monthlyRevenuePercentage = Math.round(
