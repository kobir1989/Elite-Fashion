...
const cartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      const findExistingItem = state.cartItem.find(
    ...
    removeOneFromCart: (state, action) => {
      const findById = state.cartItem.find(
    ...
    removeFromCart: (state, action) => {
      ...
      state.totalAmount =
        state.totalAmount - findById.price * findById.quantity;
      const updatedCart = state.cartItem.filter(
        (item) => item.id !== action.payload
      );
      state.cartItem = updatedCart;
...
export const { addToCart, removeOneFromCart, removeFromCart, resetCartState } =
  cartSlice.actions;
