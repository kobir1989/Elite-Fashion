...
const SubCategoryPage = () => {
  ...
  const {
    ...
    isLoading,
    isError
  } = useGetSubCategoriesQuery({ id, page })
    ...
    <PageLayout>
      <div className={styles.discount_banner}>
        <CardBanner
          ...
          subTitle={
            'Score unbeatable deals with up to 50% off at Elite Fashion!'
          }
      ...
      <GridViewLayout page={'sub_category'}>
        {!isLoading && !isError && subCategories?.subCategory?.length > 0
          ? subCategories?.subCategory.map(subCtg => (
              <CategoryCard
                title={subCtg?.name}
                imgUrl={subCtg?.image}
