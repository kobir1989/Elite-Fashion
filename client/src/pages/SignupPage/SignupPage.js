...
const SignupPage = () => {
  ...
  const [
    signup,
    { data: signupResponse, isError, isLoading, error: signupError }
  ] = useSignupMutation()
  ...
  const submitHandler = e => {
    ...
    if (password && confirmPassword) {
      if (password.length < 8) {
        return setError({
          name: 'password',
          message: 'Password should not be less then 8 Characters!'
        })
      }
      if (password !== confirmPassword) {
        return setError({
  ...
  useEffect(() => {
    if (isError) {
      ...
      createChatRoom({
        user: signupResponse?.userPayload?._id,
        admin: '63fa2849be7d427bf4c9b164'
      })
    }
