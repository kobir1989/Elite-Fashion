...
const AddReview = ({ setTabValue, getSelectedReview, selectedReviewId }) => {
  ...
  const [addReview, { isError, isLoading, error, isSuccess }] =
    useAddReviewMutation()
  const [
    updateReview,
    {
  ...
  useEffect(() => {
    ...
    if (isSuccess || updateSuccess) {
      toast.dismiss()
      toast.success(
        isSuccess ? 'Your Review has been Published' : 'Review Updated!'
      )
      setFormValues({
        comment: '',
        rating: 0
      ...
      >
        ...
        <form onSubmit={submitHandler}>
          <div className={styles.ratings}>
            <Typography variant='body' color={hasError ? 'red' : 'primary'}>
