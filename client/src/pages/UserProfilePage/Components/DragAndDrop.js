  ...
  return (
    <div>
      ...
      <div
        className={
          hasError?.file
            ? `${styles.upload_wrapper} ${styles.imageError}`
      ...
      >
        ...
        {isDragActive && (
          <div
            className={
              isDragReject ? `${styles.drag_error}` : `${styles.drag_active}`
            }
          >
            <Typography color={isDragReject ? 'red' : 'primary'}>
              {isDragReject
                ? 'Upload only .jpeg .png .jpg Image'
                : ' Drop the files here'}
            </Typography>
          </div>
        )}
        <div className={styles.upload_body}>
          {imgUrl ? (
            <div className={styles.uploaded_img}>
              ...
              >
                <Icons name={'cross'} color={'#cc2121'} size={'1rem'} />
              </Button>
              <img src={imgUrl} alt='profilePic.png' />
            </div>
          ) : (
            <>
              <Icons name={'camera'} size={'2rem'} color={'#aeb4be'} />
