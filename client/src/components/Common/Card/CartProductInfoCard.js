...
const CartProductInfoCard = ({
  ...
  return (
    <div
      className={`${styles.card_wrapper} ${styles[`color_${color}`]}`}
      onMouseEnter={() => {
        setShowDeleteBtn(!disabled && true)
      }}
      onMouseLeave={() => {
        setShowDeleteBtn(!disabled && false)
      }}
    >
      {showDeleteBtn && (
      ...
      <div className={styles.product_details_wrapper}>
        ...
        <Typography variant={'body'}>Size: {item?.size}</Typography>
        <Typography variant={'body'}>Color: {item?.color}</Typography>
