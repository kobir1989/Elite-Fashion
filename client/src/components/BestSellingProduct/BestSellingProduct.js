  ...
  return (
    <div className={styles.best_selling_product_wrapper}>
      ...
      <div className={styles.slider_wrapper}>
        <Slider {...settings}>
          {!isLoading && !isError && bestSellingProduct?.products?.length
            ? bestSellingProduct?.products.map(product => (
                <div
                  className={styles.best_selling_product_card}
                  key={product?._id}
                >
                  <ProductCard
                    title={product?.title}
      ...
      {isLoading && (
        <div className={styles.skeleton}>
          <CardSkeleton col={4} text={true} width={'20rem'} height={'22rem'} />
        </div>
      )}
